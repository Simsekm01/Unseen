...
--  Dieses Skript ist idempotent und kann beliebig oft ausgefuehrt werden.
-- ============================================================
create extension if not exists pgcrypto with schema extensions;

-- ============================================================
--  Aufraeumen (idempotent) — Policies & Funktionen vor Neuanlage loeschen
-- ============================================================
do $$
declare
  r record;
...
drop function if exists public.verify_wedding_password(uuid, text) cascade;
drop function if exists public.verify_wedding_password_by_slug(text, text) cascade;
drop function if exists public.admin_register(text, text, text) cascade;
...
drop function if exists public.check_slug_available(text) cascade;
drop function if exists public.create_wedding(text, text, text, text, date, timestamptz, timestamptz, timestamptz, boolean, timestamptz, int, text, text, text, uuid) cascade;
drop function if exists public.list_albums(uuid) cascade;
...
returns boolean language sql security definer set search_path = public, extensions as $$
  select exists (
    ...
    where id = p_wedding_id and password_hash = extensions.crypt(p_password, password_hash)
  );
$$;
...
returns boolean language sql security definer set search_path = public, extensions as $$
  select exists (
    select 1 from public.weddings
    where slug = p_slug and password_hash = extensions.crypt(p_password, password_hash)
  );
$$;
...
begin
  insert into public.weddings (
    ...
    extensions.crypt(p_password, extensions.gen_salt('bf')), p_owner_id
  ) returning id into v_id;
